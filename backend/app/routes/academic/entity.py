...
def give_scores_route(validated_data):
    ...
    msg = f'Đã cho điểm cho học sinh!'
...
def summary_lesson_for_student(validated_data):
    result = score_workflow.process_summary_semester_lessons_result(validated_data, get_jwt().get('id'))
    msg = f'Đã tổng kết điểm cho lớp {result.class_room}!'
    return ResponseBuilder.put(msg)
...
def create_schedule_route(validated_data):
    academic_schedule_service.handle_upsert_schedule(validated_data)
...
def show_schedules_in_tool(validated_data):
    result = academic_schedule_service.handle_show_schedules_in_tool(validated_data)
...
@academic_entity_bp.get('attendence/schedules')
@jwt_required()
@required_role('admin', 'Teacher')
@validate_input(AcademicShowSchemas.Schedule)
def get_schedules_by_teacher_day(validated_data):
    validated_data['user_id'] = get_jwt().get('id')
    result = academic_schedule_service.handle_show_schedules_for_attendence(validated_data)
    msg = 'Không tìm thấy thông tin!'
    return ResponseBuilder.get(msg, result)

@academic_entity_bp.get('schedules/today')
@jwt_required()
@required_role('admin')
@validate_input(AcademicShowSchemas.Schedule)
def get_today_schedules(validated_data):
    result = academic_schedule_service.handle_show_today_schedules(validated_data)
    msg = 'Không tìm thấy thông tin!'
    return ResponseBuilder.get(msg, result)